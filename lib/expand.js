...
expand.write = function(data, el, targetEl, opts) {
  ...
  if(data.id === null) {
    ...
    wrapper.className = "img-expander-wrapper";
    wrapper.style.position = "relative";
    wrapper.style.width = "100%";
    wrapper.style.height = "100%";

  } else {
    wrapper = store[data.id].wrapper
    childEl = wrapper.firstChild;
  }
  ...
  document.body.appendChild(store[data.id].wrapper);
  ...
  childEl.style[stylePrefixer("transform")] = "translate("+data.x+"px, "+data.y+"px) scale("+(1/scale)+")";
  setTimeout(function() {
    childEl.style[stylePrefixer("transform")] = "translate("+tx+"px, "+ty+"px) scale(1)";
  }, 0);
