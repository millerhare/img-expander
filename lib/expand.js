...
expand.write = function(data, el, targetEl, opts) {
  ...
  childEl.style.width          = data.cw+"px";
  childEl.style.height         = data.ch+"px";
  ...
  if(opts.animation) {
    childEl.style.webkitTransition =
      stylePrefixer("transform")+" "+interval+"ms,"+stylePrefixer("background-position")+" "+interval+"ms";
  }

  if(data.id === null) {
    // Don't do it again!
    // childEl.style[stylePrefixer("transform")] = "translate("+data.x+"px, "+data.y+"px,0)";
    childEl.style.backgroundPosition = data.x+"px "+data.y+"px";
  }
  ...
  var scalew = data.cw / data.w;
  var scaleh = data.ch / data.h;
  var scale = 1;
  ...
  childEl.setAttribute('data-return-to-scale', 1/scale);
  childEl.style[stylePrefixer("transform")] = "scale("+(1/scale)+")";
  childEl.style[stylePrefixer("backgroundPosition")] = data.x+"px "+data.y+"px";
  setTimeout(function() {
    childEl.style[stylePrefixer("transform")] = "scale(1)";
    childEl.style[stylePrefixer("backgroundPosition")] = tx+"px "+ty+"px";
  }, 0);
